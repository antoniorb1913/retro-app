...
import type { GameWrite } from '../../../models/game.interface';
...
export class GameFormComponent implements OnInit, OnDestroy {
  ...
  private loadItem(): void {
    this.gameService.getById(this.id).subscribe({
      next: (game) => {
        this.form.patchValue({ name: game.name, edition: game.edition || '', model: game.model || '', platform: game.platform || '', region: game.region, status: game.status, price: game.price ? Number(game.price) : null, acquisition_date: game.acquisition_date || '', description: game.description || '', complete: game.complete });
        this.selectedComponentIds.set(game.missing_components.map((c) => c.id));
        this.images.set(game.images);
        this.syncComponentsDisabled();
      },
      error: () => this.error.set('Error al cargar el juego'),
    });
  }

  protected loadImages(): void { if (!this.id) return; this.gameService.getById(this.id).subscribe({ next: (item) => this.images.set(item.images) }); }
  ...
  protected onCreateUpload(files: FileList | null): void {
    ...
    this.gameService.create(this.buildData()).subscribe({
      next: (result) => {
        const formData = new FormData();
        formData.append('image', files[0]);
        formData.append('content_type_model', 'game');
        formData.append('object_id', String(result.id));
        this.api.upload('images', formData).subscribe({
          next: () => this.router.navigate(['/games', result.id, 'edit']),
          error: () => this.router.navigate(['/games', result.id, 'edit']),
        });
      },
  ...
  protected onSubmit(): void {
    ...
    const request = this.isEdit() ? this.gameService.update(this.id, this.buildData()) : this.gameService.create(this.buildData());
    request.subscribe({
      next: (result) => {
        this.submitting.set(false);
        if (this.isEdit()) { this.router.navigate(['/games', result.id]); }
        else { this.id = result.id; this.isEdit.set(true); window.history.replaceState({}, '', `/games/${result.id}/edit`); this.loadItem(); }
      },
